...
安装程序包含CPU引擎和文档处理库。CUDA和Vulkan引擎会在首次使用时自动下载：**CUDA约537 MB，Vulkan约34 MB**。下载支持暂停和断点续传，已准备好的引擎可在应用更新后继续使用。自动模式选择一个兼容的GPU引擎，必要时回退到CPU。

无需另外安装Rust、Python、Node.js、LM Studio或CUDA Toolkit。GPU处理需要兼容的显卡驱动程序。
