...
if(!/^v\d+\.\d+\.\d+$/.test(tag||'')) throw new Error('Expected a release tag such as v1.0.0');
...
lock=lock.replace(/(name = "glyph-ocr"\r?\nversion = ")[^"]+/,(_,prefix)=>prefix+version);
await writeFile('src-tauri/Cargo.lock',lock);
console.log('Building Glyph OCR '+version+' from '+tag);
